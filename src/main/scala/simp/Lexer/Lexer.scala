...
class Lexer(source: String, sourceLines: List[String]):
    ...
    private def scanToken(): Unit = {
        ...
        val token : Token = peek() match {
            ...
            case x if isWordMatch("Float") => {advanceUntilNextWord(); Token.TypeFloat}
            ...
            case x if isWordMatch("impl") => { advanceUntilNextWord(); Token.Impl }

            case x if isWordMatch("priv") => { advanceUntilNextWord(); Token.Priv }
